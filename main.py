...
Join @TheChampu For Updates
[Source Code](https://github.com/TheChampu/TeraBoxDL) """
    check_if = await is_user_on_chat(bot, "@TheChampu", m.peer_id)
    if not check_if:
        return await m.reply("Please join @TheChampu then send me the link again.")
    check_if = await is_user_on_chat(bot, "@ShivanshuHUB", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @ShivanshuHUB then send me the link again."
        )
...
Size: **{data["size"]}** 
Direct Link: [Click Here](https://t.me/itsTeraBoxRobot?start={uuid})
